...
async def _(event):
    sbb_bevent = await edit_or_reply(event, "** ※ جار انشاء رابط تلجراف**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f" ※ انشاء حساب تلجراف جديد {auth_url} لهذه الجلسة. \n**لا تعطي هذا الرابط لاي احد, حتى لو قالو انهم من شركة التلكرام!**",
        )
    optional_title = event.pattern_match.group(5)
    if not event.reply_to_msg_id:
        return await sbb_bevent.edit(
            "** ※ قم بالرد على هذه الرسالة للحصول على رابط تلجراف فورا**",
        )
