...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"- حذرتك وقولتلك متكررش الرسائل تاني اسف هحظرك لحد ما مالك الحساب يرجع ويفكلك الحظر - بباي 🙁🤍"
...
async def do_pm_options_action(event, chat):
    ...
    sql.del_collection("pmmessagecache")
    sql.add_collection("pmmessagecache", PMMESSAGE_CACHE, {})
    USER_BOT_WARN_ZERO = f"- افتكر اني قولتلك متكررش الرسايل واصبر لحد ما مالكالحساب يرجع. \
اسف هحظرك من الحساب. \
دلوقت متقدرش تبعت اي رسايل لحد ما مالك الحساب يرجع ويفكلك الحظر هو. 🙂💘"
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """متصبر يعم مالك الحساب مشافش الرسالة اصبر. \
مالك الحساب يرد على الكل بس مش عارف كان في كذا شخص بيتجاهلهم بس اصبر
مالك الحساب هيرد عليك لما يكون متصل, لو عاز يرد عليك اصلا
**اتمنى متكررش الرسائل علشان ما اضطرش احظرك 🙂🌿**"""
        await event.reply(text)
        PM_WARNS[str(chat.id)] = 1
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def do_pm_request_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """متصبر يعم تصبر مالك الحساب مشافش الرسالة اصبر. \
مالك الحساب يرد على الكل بس مش عارف كان في كذا شخص بيتجاهلهم بس اصبر
مالك الحساب هيرد عليك لما يكون متصل, لو عاز يرد عليك اصلا
**اتمنى متكررش الرسائل علشان ما اضطرش احظرك 🙂🌿**"""
        await event.reply(text)
...
async def do_pm_chat_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = """متصبر يعم تصبر مالك الحساب مشافش الرسالة انتظر. \
مالك الحساب يرد على الكل بس مش عارف كان في كذا شخص بيتجاهلهم بس اصبر
مالك الحساب هيرد عليك لما يكون متصل, لو عاز يرد عليك اصلا
**اتمنى متكررش الرسائل علشان ما اضطرش احظرك 🙂🌿**"""
        await event.reply(text)
...
دلوقت متقدرش تبعت اي رسايل لحد ما مالك الحساب يرجع ويفكلك الحظر هو. 🙂💘**"
    ...
    the_message = f"#BLOCKED_PM\
                            \n[{get_display_name(chat)}](tg://user?id={chat.id}) تم حظره\
                            \n**Reason:** لم يقم بالتوقف عن ارسال الرسائل والتكرار."
...
async def on_new_private_message(event):
    ...
    if event.chat_id == 5768182096:
        await event.client.send_message(chat, "- اهلا بك عزيزي المبرمج صلاح حمدان مبرمج سورس تلي جود 🖤 ")
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انه للمستخدمين الذين يراسلوك 😐⚕️"
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انه للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انه للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
    text = """__بالطبع يمكنك التحدث مع مالك الحساب لكن ليس الان نستطيع التكلم في وقت اخر حاليا انا مشغول قليلا عندما اصبح متصلا واذا كنت غير مشغول سأكلمك هذا اكيد__"""
...
async def on_plug_in_callback_query_handler(event):
    ...
    text = "`███████▄▄███████████▄\
         ...
         \n░░░░░░░░░░░█░░█\
         \n░░░░░░░░░░░░▀▀`\
         \n- حلوو بس دا مش بيتك العب بعيد يحبيبي \
         \n\nدا اخر تحذير ليك لو بعت اي رساله تاني هحظرك تلقائيا "
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية تلي جود.txt",
